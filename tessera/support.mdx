...
<AccordionGroup>
  <Accordion title="Commands do nothing">
    Check the module is enabled in `=config`, and that Tessera's role sits **above** any role it
  ...
  <Accordion title="The bot left the server">
    During the alpha, Tessera leaves servers that are not on the access list, and posts the
...
Tessera only joins servers on the access list during the alpha. Ask for access **before**
...
Note that Tessera can also be put into **maintenance mode** deliberately, during which commands
